...
const img = {
  hero: "/images/mountain-view-hero.png",
  stem: "/images/mountain-view-learning.png",
  social: "/images/mountain-view-community.png",
  arts: "/images/mountain-view-community.png",
  campus: "/images/mountain-view-hero.png",
};
...
function Pathways({ onNotify }: { onNotify: Notify }) {
  return <section id="pathways" className="bg-[#0b1f3a] py-24 text-[#faf8f4] md:py-32"><div className="container"><SectionIntro dark eyebrow="Senior School · Grades 10—12" title={<>Find your <span className="text-[#dec27f]">path.</span></>} copy="Three pathways. A hundred ways to become more yourself. Our guidance team helps every learner choose with clarity, not pressure." action={<a href="/academics" className="focus-ring inline-flex items-center gap-3 text-[11px] font-bold uppercase tracking-[.14em] text-[#dec27f]">Compare pathways <ArrowRight size={15} /></a>} /><div className="grid gap-4 lg:grid-cols-3">{pathways.map(({ title, label, image, icon: Icon, copy, subjects }, i) => <article key={title} className="tilt-card group relative min-h-[540px] overflow-hidden bg-[#183254]"> <img src={image} alt={`${title} pathway at Mountain View`} className="absolute inset-0 h-full w-full object-cover opacity-75 transition duration-500 group-hover:scale-105 group-hover:opacity-90" /><div className="absolute inset-0 bg-gradient-to-t from-[#061428] via-[#0b1f3a]/65 to-transparent" /><div className="relative flex min-h-[540px] flex-col justify-end p-7 md:p-9"><div className="mb-auto flex items-start justify-between"><span className="grid h-11 w-11 place-items-center border border-white/35 bg-[#0b1f3a]/25"><Icon size={20} strokeWidth={1.4} /></span><span className="text-[10px] font-bold uppercase tracking-[.18em] text-[#dec27f]">0{i + 1}</span></div><div className="eyebrow mb-3 text-[#dec27f]">{label}</div><h3 className="font-display text-4xl font-semibold">{title}</h3><p className="mt-4 text-sm leading-6 text-white/72">{copy}</p><div className="mt-5 border-t border-white/20 pt-4 text-[10px] font-bold uppercase tracking-[.11em] text-white/55">{subjects}</div><button onClick={() => onNotify(`${title} pathway details are ready for your enquiry.`)} className="focus-ring mt-7 flex items-center gap-3 text-[10px] font-bold uppercase tracking-[.16em] text-[#dec27f]">Explore pathway <ArrowRight size={15} /></button></div></article>)}</div></div></section>;
}
